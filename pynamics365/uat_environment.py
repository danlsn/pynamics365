...
from ddb_loader import load_many_entity_records_to_ddb, get_entity_definitions, extract_all_pages_to_ddb
from pynamics365.config import UAT_DEFS_PATH
...
def main():
    ...
    entity_definitions = load_definitions(defs_path=UAT_DEFS_PATH)
    logical_names = entity_definitions.index.tolist()
    for logical_name in logical_names[500:]:
        try:
            primary_key = entity_definitions.loc[logical_name].PrimaryIdAttribute
            extract_all_pages_to_ddb(dc, logical_name, primary_key=primary_key, chunk_size=200_000_000)
        except KeyError:
    ...
    df.set_index(["LogicalName", "SchemaName", "LogicalCollectionName", "ObjectTypeCode", "IsLogicalEntity",
                 ...
                 inplace=True)
    output_path = output_path / resource_path_name
...
if __name__ == "__main__":
    load_dotenv()
    entity_list = []
    full_extract_to_ddb(entity_list=None, resource=os.getenv("MSDYN_UAT_RESOURCE"), multi_threaded=True)
    main()
